...
bool LoadObj(const std::string& basepath, const std::string& filename,
    rpr_context context, rpr_material_system matsys, rpr_scene scene)
{
    rpr_int status = RPR_SUCCESS;
...
void LoadObjTest(const std::string& basepath, const std::string& filename)
{
    ...
    if (!LoadObj(basepath, filename, context, matsys, scene))
    {
        std::cerr << "Error: failed to load " + basepath + "/" + filename + "scene." << std::endl;
        assert(false);
    }
